...
\begin{enumerate}[label=(\roman*)]
    ...
    \begin{equation} \label{eq:limit-of-fn}
        f_n(g) = f_n(\exp Y_1)^{m_1} \cdots f_n(\exp Y_k)^{m_k} \to (\exp \psi(Y_1))^{m_1} \cdots (\exp \psi(Y_k))^{m_k}
    \end{equation}
    ...
    \end{equation*}
    Therefore, $\psi \in D(\Aut(G))$ and hence $D(\Aut(G))$ is closed.
