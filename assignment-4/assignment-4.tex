...
\begin{enumerate}[label=(\roman*)]
    ...
    \end{equation*}
    But by (ii), $x \mapsto \langle R_x(f), f_j \rangle$ belongs to $\mathcal{R}(G)$ for all $j$. Therefore, $f \in \mathcal{R}(G)$. This holds for any $f \in \mathcal{R}(G)^\perp$, so $\mathcal{R}(G)^\perp \subset \mathcal{R}(G)$. It follows that $C(G) = \mathcal{R}(G) \oplus \mathcal{R}(G)^\perp = \mathcal{R}(G)$.
\end{enumerate}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item Let $O \subset \mathfrak{g}$ be an open neighbourhood of $0$ such that $\exp : O \to \exp(O)$ is a diffeomorphism. Since $f_n(\exp X) = \exp(T_e f_n (X)) \to \exp(\psi(X))$ as $n \to \infty$ by the continuity of $\exp$, for all $X \in O$, the sequence $(f_n)_n$ converges pointwise on $\exp(O)$. Now fix $g \in G$, not necessarily in $\exp(O)$. Then there exist $X_1, \ldots, X_k \in \mathfrak{g}$ such that $g = \exp(X_1) \cdots \exp(X_k)$. For all $j \in \{1, \ldots, k\}$, Let $m_j \in \bb{N}$ be large enough that $X_j/m_j \in O$. Write $Y_j = X_j/m_j$ for all $j$. Then, by Proposition 2.9 (Koelink),
    \begin{equation*}
        g = \exp(m_1Y_1) \cdots \exp(m_kY_k) = (\exp Y_1)^{m_1} \cdots (\exp Y_k)^{m_k}.
    \end{equation*}
    Now we see that
    \begin{equation} \label{eq:limit-of-fn}
        f_n(g) = f_n(\exp Y_1)^{m_1} \cdots f_n(\exp Y_k)^{m_k} \to (\exp \psi(Y_1))^{m_1} \cdots (\exp \psi(Y_k))^{m_k} \text{ as } n \to \infty
    \end{equation}
    ...
    We now show that $f$ is a group automorphism of $G$. Let $g, h \in G$. Then
    \begin{equation*}
        f(gh) = \lim_{n \to \infty} f_n(gh) = \lim_{n \to \infty} f_n(g)f_n(h) = f(g)f(h)
    \end{equation*}
    by the continuity of multiplication, so $f$ is a group homomorphism. Let $F : G \to G$ be the function that takes $g = (\exp Y_1)^{m _1}\cdots (\exp Y_k)^{m_k}$ to
    \begin{equation*}
        F(g) = (\exp (-\psi(Y_k)))^{m_k} \cdots (\exp (-\psi(Y_1)))^{m_1}.
    \end{equation*}
    By Proposition 2.9, $f \circ F = F \circ f = \mathrm{id}_G$, so $f$ is bijective. Note that $F$ is well-defined: if
    \begin{equation*}
        g = (\exp Y_1)^{m_1} \cdots (\exp Y_k)^{m_k} = (\exp Z_1)^{p_1} \cdots (\exp Z_l)^{p_l}
    \end{equation*}
    for $Y_1, \ldots, Y_k, Z_1, \ldots, Z_l \in O$, and $m_1, \ldots, m_k, p_1, \ldots, p_l \in \bb{N}$, we know by the uniqueness of the limit \eqref{eq:limit-of-fn} that
    \begin{equation*}
        (\exp \psi(Y_1))^{m_1} \cdots (\exp \psi(Y_k))^{m_k} = (\exp \psi(Z_1))^{p_1} \cdots (\exp \psi(Z_l))^{p_l}
    \end{equation*}
    and hence by taking the inverse,
    \begin{equation*}
        (\exp (- \psi(Y_k)))^{m_k} \cdots (\exp (- \psi(Y_1)))^{m_1} = (\exp (-\psi(Z_l)))^{p_l} \cdots (\exp (-\psi(Z_1)))^{p_1}.
    \end{equation*}
    ...
    \item The map $\exp \circ \, \psi \circ \exp^{-1} : \exp(O) \to \exp(\psi(O))$, with $O \subset \mathfrak{g}$ the neighbourhood of 0 from (ii), is continuous at $e$ since $\exp$, $\psi$ and $\exp^{-1}$ are all continuous on the domains we are considering here. Now fix $g = (\exp Y_1)^{m _1}\cdots (\exp Y_k)^{m_k} \in G$. Then $l_g(\exp(O))$ is an open neighbourhood of $g$. For any $h \in l_g(\exp(O))$, we have
    \begin{equation*}
        h = gg^{-1}h = (\exp Y_1)^{m _1}\cdots (\exp Y_k)^{m_k} \exp((\exp^{-1} \circ \, l_{g^{-1}})(h)).
    \end{equation*}
    Note that $(\exp^{-1} \circ \, l_{g^{-1}})(h) \in O$. Using the construction in (ii), we find
    \begin{align*}
        f(h) &=  (\exp \psi(Y_1))^{m _1}\cdots (\exp \psi(Y_k))^{m_k} (\exp \circ \,\psi \circ \exp^{-1} \circ \, l_{g^{-1}})(h) \\
        &= f(g) (\exp \circ \,\psi \circ \exp^{-1} \circ \, l_{g^{-1}})(h)
    \end{align*}
    This holds for all $h \in l_g(\exp(O))$, so
    \begin{equation*}
        f =  f(g) \cdot \exp \circ \,\psi \circ \exp^{-1} \circ \, l_{g^{-1}}
    \end{equation*}
    on $l_g(\exp(O))$. Since $l_{g^{-1}}(g) = e$ and $l_{g^{-1}}$ is continuous at $g$, the map $\exp \circ \,\psi \circ \exp^{-1} \circ \, l_{g^{-1}}$ is continuous at $g$. Group multiplication is also continuous, and therefore $f$ is continuous at $g$. This holds for any $g \in G$, so $f$ is continuous.

    By Proposition 5.1 (Koelink), the continuity of $f$ implies that $f$ is smooth.

    \item Let $(\psi_n)_{n \in \bb{N}}$ be a sequence in $D(\Aut(G))$ that converges to $\psi \in \Aut(\mathfrak{g})$. Then, for every $n \in \bb{N}$, there exists an $f_n \in \Aut(G)$ such that $\psi_n = T_ef_n$. By (ii), there exists a group automorphism $f : G \to G$ for which $f(\exp(X)) = \exp(\psi(X))$ for all $X$ in a small enough open neighbourhood of $0 \in \mathfrak{g}$. Part (iii) says that $f$ is a Lie group automorphism, so $f \in \Aut(G)$. Furthermore, for $X \in \mathfrak{g}$,
    \begin{equation*}
        T_e f(X) = \frac{d}{dt}\bigg\vert_{t=0} f(\exp(tX)) = \frac{d}{dt}\bigg\vert_{t=0} \exp(\psi(tX)) = \psi(X).
    \end{equation*}
    Therefore, $\psi \in D(\Aut(G))$ and hence $D(\Aut(G))$ is closed in $\Aut(\mathfrak{g})$.
    \item We will show that $\dim \Aut(S^1) < \dim \Aut(i\bb{R})$, where $i\bb{R}$ is the Lie algebra of $S^1$.
    
    First, it is clear that the only group homomorphisms of $i\bb{R}$ (which is isomorphic to $\bb{R}$) are of the form $iX \mapsto \lambda iX$ with $\lambda \in \bb{R}$. The automorphisms are then the maps $iX \mapsto \lambda iX$ with $\lambda \in \bb{R} \setminus \{0\}$. Hence, the automorphism group is the one-dimensional Lie group $\bb{R}^\times$.
    
    Next, for $iX \in i\bb{R}$ define $\alpha_{iX} : \bb{R} \to S^1 : t \mapsto e^{itX}$. The latter is clearly a one-parameter subgroup with $\alpha_{iX}(0) = 0$ and $\frac{d}{dt}\big\vert_{t=0} \alpha_{iX}(0) = iX$. Therefore, the map $i\bb{R} \to S^1 : iX \mapsto \alpha_{iX}(1) = e^{iX}$ works as the exponential map. In this case the exponential map is surjective.

    Now let $f \in \Aut(S^1)$. Then $f(e^{iX}) = e^{T_ef(iX)}$. We know $T_ef \in \Aut(i\bb{R})$, so we must have $T_ef(iX) = \lambda iX$ for some $\lambda \in \bb{R}^\times$. Then, $f(e^{iX}) = e^{\lambda iX}$. However, since $f$ is a homomorphism, $1 = f(1) = f(e^{i2\pi}) = e^{\lambda i2\pi}$, so $\lambda \in \bb{Z}$. Moreover, $f$ must be injective, so we want
    \begin{equation*}
        \ker f = \{e^{iX} \mid e^{\lambda iX} = 1\} = \{ e^{iX} \mid \lambda X \in 2\pi \bb{Z} \} = \{1\}.
    \end{equation*}
    In other words, we want $\lambda X \in 2\pi \bb{Z}$ to imply $X \in 2\pi\bb{Z}$. The only integers for which this holds are $\lambda = \pm 1$. These are all the restrictions we need, since the maps $e^{iX} \mapsto e^{iX}$ and $e^{iX} \to e^{-iX}$ are indeed both automorphisms of $S^1$. This is a group of order two, so $\Aut(S^1) \cong \bb{Z}^2$. In particular, $\dim \Aut(S^1) = 0$.
