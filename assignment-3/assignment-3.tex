...
\begin{enumerate}[label = (\roman*)]
    ...
    \end{equation*}
    so $\mu_g$ is left-invariant. In the second equality we used that $l_g(r_{g^{-1}}(x)) = gxg^{-1} = r_{g^{-1}}(l_g(x))$ for all $x \in G$, and in the third equality we used the left invariance of $\mu$. Since $\mu$ is a left-invariant Haar measure on $G$, by Corollary 19.7 (van de Ban), $\mu_g = \Delta(g) \mu$ for some constant $\Delta(g) \in \bb{R}_{\geq 0}$ which may depend on $g$. However, $\Delta(g) \neq 0$, since $\mu_g(U) = \mu(r_{g^{-1}}(U)) > 0$ for any nonempty open $U \subset G$, as $r_{g^{-1}}(U)$ is still nonempty and open because $r_{g^{-1}}$ is a homeomorphism.

    \item Let $A \in \mathcal{B}(G)$ such that $\mu(A) > 0$, and let $g,h \in G$. Note that $\mu(r_{x^{-1}}(A)) = \mu_x(A) = \Delta(x)\mu(A)$ for any $x \in G$. Therefore,
    \begin{align*}
        \Delta(gh)\mu(A) = \mu(r_{(gh)^{-1}}(A)) = \mu(r_{g^{-1}}(r_{h^{-1}}(A))) = \Delta(g)\mu(r_{h^{-1}}(A)) = \Delta(g)\Delta(h)\mu(A).
    \end{align*}
    Since $\mu(A) > 0$, $\Delta(gh) = \Delta(g)\Delta(h)$. But $g, h \in G$ were arbitrary, so $\Delta : G \to \bb{R}_{>0}$ is a group homomorphism.

    \item Let $K$ be a compact subgroup of $G$. By (iii), it suffices to show that $\Delta(K)$ is bounded, since $\{1\}$ is the only bounded subgroup of $\bb{R}_{>0}$. Suppose, for the sake of contradiction, that $\Delta(K)$ is unbounded. Then there exists a sequence $(g_n)_n$ in $K$ such that $\Delta(g_n) \to \infty$ as $n \to \infty$. Since $K$ is a subgroup, $K{g_n}^{-1} = K$ for every $n$. Therefore,
    \begin{equation*}
...
\begin{enumerate}[label = (\roman*)]
    ...
    \item Suppose that there is an invariant subspace $W \subsetneq V$ and $W \neq \{0\}$. Let $w \in W$. Then, for every $g \in \mathrm{SO}(2)$, there exists a $\lambda_g \in \bb{R}$ such that $gw = \lambda_g w$, since $W$ must be one-dimensional. In other words, $w$ is a joint eigenvector of all elements of $\mathrm{SO}(2)$, always with a real eigenvalue. In particular, there exists such a $\lambda_g$ for the element $g \in \mathrm{SO}(2)$ represented by
    \begin{equation*}
        \begin{pmatrix}
            0 & -1 \\ 1 & 0
        \end{pmatrix}.
    \end{equation*}
    However, the eigenvalues of this matrixare $i$ and $-i$, neither of which are real. This is a contradiction, so it follows that such a subspacee $W$ cannot exist. In other words, the only invariant subspaces of $\bb{R}^2$ are $\{0\}$ and $\bb{R}^2$ itself, so the representation is irreducible.
\end{enumerate}
