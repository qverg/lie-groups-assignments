...
\begin{enumerate}[label = (\roman*)]
    \item Suppose $A \in \mathcal{B}(G)$. Let $g \in G$. Since $l_g$ is a homeomorphism and $\mathcal{B}(G)$ is fully specified by the topology on $G$, $l_g(A) \in \mathcal{B}(l_g(G))$. But $l_g(G) = G$ as topological spaces, so $\mathcal{B}(l_g(G)) = \mathcal{B}(G)$. The reasoning for $r_g$ is the same.
    
    \item Let $A \in \mathcal{B}(G)$ and $g \in G$. Then
    \begin{equation*}
        \mu_g(l_g(A)) = \mu(r_{g^{-1}}(l_g(A))) = \mu(l_g(r_{g^{-1}}(A))) = \mu(r_{g^{-1}}(A)) = \mu_g(A),
    \end{equation*}
    so $\mu_g$ is left-invariant. In the second equality we used that $l_g(r_{g^{-1}}(x)) = gxg^{-1} = r_{g^{-1}}(l_g(x))$ for all $x \in G$, and in the third equality we used the left invariance of $\mu$. Since $\mu_g$ is a left-invariant Haar measure on $G$, by Corollary 19.7 (van de Ban), $\mu_g = \Delta(g) \mu$ for some $\Delta(g) \in \bb{R}_{\geq 0}$ which may depend on $g$. However, $\Delta(g) \neq 0$, since $\mu_g(U) = \mu(r_{g^{-1}}(U)) > 0$ for any nonempty open $U \subset G$, as $r_{g^{-1}}(U)$ is still nonempty and open because $r_{g^{-1}}$ is a homeomorphism.

    \item Let $g,h \in G$. By (ii), we have that $\mu(r_{x^{-1}}(G)) = \mu_x(G) = \Delta(x)\mu(G)$ for any $x \in G$. Therefore,
    \begin{align*}
        \Delta(gh)\mu(G) = \mu(r_{(gh)^{-1}}(G)) = \mu(r_{g^{-1}}(r_{h^{-1}}(G))) = \Delta(g)\mu(r_{h^{-1}}(G)) = \Delta(g)\Delta(h)\mu(G).
    \end{align*}
    Since $\mu(G) > 0$ ($G$ is open and nonempty in $G$), $\Delta(gh) = \Delta(g)\Delta(h)$. But $g, h \in G$ were arbitrary, so $\Delta : G \to \bb{R}_{>0}$ is a group homomorphism.

    \item Let $K$ be a compact subgroup of $G$. By (iii), it suffices to show that $\Delta(K)$ is bounded, since the image of a subgroup under a group homomorphism is a subgroup, and $\{1\}$ is the only bounded subgroup of $\bb{R}_{>0}$. Suppose, for the sake of contradiction, that $\Delta(K)$ is unbounded. Then there exists a sequence $(g_n)_n$ in $K$ such that $\Delta(g_n) \to \infty$ as $n \to \infty$. But $K$ is a group, so $K{g_n}^{-1} = K$ for every $n$. Therefore,
    \begin{equation*}
        \mu(K) = \mu(K{g_n}^{-1}) = \Delta(g_n)\mu(K) \to \infty \quad \text{as } n \to \infty,
    \end{equation*}
    but $\mu(K)$ does not depend on $n$, so $\mu(K) = \infty$. This contradicts the given property that the $\mu$-measure of every compact subset of $G$ is finite.
\end{enumerate}

\section*{Week 9, Exercise 3}
\begin{enumerate}[label=(\roman*)]
    \item We take $C(G)$ to be equipped with the supremum norm $\lVert \, \cdot \, \rVert_\infty$. For any $g \in G$, the map $L_g : V \to V$ is linear. Since $V$ is a finite-dimensional normed space, $L_g$ is continuous for any $g \in G$. Now fix $\eps >0$ and select $(h, \psi) \in G \times V$. We want to show that there exists an open neighbourhood of $(h, \psi)$ such that $\lVert L(g, \phi) - L(h,\psi) \rVert_\infty < \eps$ for all $(g, \phi)$ in that neighbourhood, which would imply that $L : G \times V \to V$ is continuous at $(h,\psi)$.
    ...
    \begin{align*}
        ...
        &< \eps/2,
    \end{align*}
    where we used that $\{g^{-1}x \mid x \in G\} = G = \{h^{-1}x \mid x \in G\}$ since the left multiplications $\ell_{g^{-1}}$ and $\ell_{h^{-1}}$ are bijections. Simiarly, for any $g \in G$,
    \begin{equation*}
    ...
    \begin{lemma} \label{lemma1}
        Let $G$ be a topological group. For any open neighbourhood $E \subset G$ of $e$, there exists an open neighbourhood $A$ of $e$ contained in $E$ such that $AA^{-1} \subset E$.\footnote{Here, $AA^{-1}$ means $\{xy^{-1} \mid x, y \in A\}$.}
    \end{lemma}
    \begin{proof}
        The map
        \begin{equation*}
    ...
    Fix $f \in C(G)$ and select $\eps >0$. By part (ii), there exists a neighbourhood $W$ of $e \in G$ such that
    \begin{equation*}
        \lvert L_g(f)(x) - L_e(f)(x) \rvert = \lvert f(g^{-1}x) - f(x) \rvert < \eps
    \end{equation*}
    for all $x \in G$, $g \in W$. Indeed, we can take $W= U^{-1}$, where $U$ is the neighbourhood of $e$ as in (ii). The set $W$ is open since $W = \iota^{-1}(U)$ where $\iota$ is the inverse operation on $G$, which is continuous, and $e \in W$ since $e^{-1} = e$. It follows that
    \begin{equation*}
        \lVert L_g(f)- L_e(f) \rVert_\infty = \sup_{x \in G} \lvert L_g(f)(x) - L_e(f)(x) \rvert \leq \eps
    \end{equation*}
    for all $g \in W$. Since $\eps>0$ was arbitrary, we have found that $g \mapsto L_g(f)$ is continuous at $e$.  By Proposition 20.8, $L : G \times C(G) \to C(G)$ is continuous, so the representation $L$ of $G$ on $C(G)$ is continuous.    
\end{enumerate}
...
\begin{enumerate}[label = (\roman*)]
    ...
    \end{equation*}
    In the first and last equality we used the definition of $\pi$; in the second and fourth equality we used that $\pi$ is a group action; and in the third equality we used that $G$ is commutative. It follows that $\pi(g) : V \to V$ is an equivariant map. Then, by Schur's lemma, $\pi(g) = \lambda_g I_V$ for some $\lambda_g \in \bb{C}$. But this means that $\pi(g)$ leaves every subspace of $V$ invariant. This holds for every $g \in G$. Hence, if $\pi$ is irreducible, then $\dim V = 1$, since in that case the only linear subspaces of $V$ are $\{0\}$ and $V$ itself. Conversely, if $\dim V = 1$, then the only linear subspaces of $V$ are $\{0\}$ and $V$ itself, so $\pi$ is trivially irreducible.
    ...
    \begin{equation*}
        ...
        \end{pmatrix}
    \end{equation*}
    is symmetric in $\theta$ and $\phi$.
    Therefore, we can use (i): $\dim \bb{C}^2 > 1$, so the representation is not irreducible.

    \item Suppose that there is an invariant subspace $W \subsetneq \bb{R}^2$ and $W \neq \{0\}$ (in terms of vector spaces over $\bb{R}$). Let $w \in W$. Then, for every $g \in \mathrm{SO}(2)$, there exists a $\lambda_g \in \bb{R}$ such that $gw = \lambda_g w$, since $W$ must be one-dimensional. In other words, $w$ is a joint eigenvector of all elements of $\mathrm{SO}(2)$, always with a real eigenvalue. In particular, there exists such a $\lambda_g$ for the element $g \in \mathrm{SO}(2)$ represented by
    \begin{equation*}
        \begin{pmatrix}
            0 & -1 \\ 1 & 0
        \end{pmatrix}.
    \end{equation*}
    However, the eigenvalues of this matrix are $i$ and $-i$, neither of which are real. This is a contradiction, so it follows that such a subspace $W$ cannot exist. In other words, the only invariant subspaces of $\bb{R}^2$ are $\{0\}$ and $\bb{R}^2$ itself, so the representation is irreducible.
\end{enumerate}
