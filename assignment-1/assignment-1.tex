...
\begin{enumerate}[label=(\roman*)]
    ...
    \begin{equation*}
        \lambda_i |v_i|^2 = \langle v_i, \lambda_i v_i \rangle = \langle v_i, x^tx v_i \rangle = \langle xv_i, xv_i \rangle = |xv_i|^2,
    \end{equation*}
    ...
    \begin{align*}
        ...
        &= \sum_{i=1}^{n} \ln(\sqrt{\lambda_i}) \\
        &= \ln(\sqrt{\lambda_1 \cdots \lambda_n}) = \ln (\sqrt{\det(x^tx)}) = \ln(1) = 0.
    \end{align*}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \end{align*}
    where we used the fact that $v_X$ is left-invariant in the third equality. By uniqueness of integral curves, and since the integral curve of a left-invariant vector field is determined by its value at $t=0$, $\ell_{g^{-1}} \circ \Phi^t_X(g)$ is the integral curve for $v_X$. Therefore, by Proposition 2.9(i) in the lecture notes, $\ell_{g^{-1}} \circ \Phi^t_X(g) = \exp(tX)$, so $\Phi^t_X(g) = g\exp(tX)$.
    ...
    \begin{align}
        ...
        &= (\d_{gh \exp(tX)} r_{\exp(-tX)} \circ \d_{h \exp(tX)} \ell_g) (v_Y(h\exp(tX))), \label{eq1}
    \end{align}
    ...
    \end{equation*}
    Applying this to \eqref{eq1}, we get
    \begin{align*}
